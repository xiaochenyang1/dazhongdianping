...
CREATE TABLE IF NOT EXISTS shop_search_sync_task (
...
CREATE INDEX IF NOT EXISTS idx_shop_search_sync_dispatch
    ON shop_search_sync_task(status, next_retry_at, locked_at, shop_id);
...
CREATE INDEX IF NOT EXISTS idx_coupon_remind ON coupon(status, remind_status, expire_at, id);
CREATE TABLE IF NOT EXISTS refund (id BIGINT AUTO_INCREMENT PRIMARY KEY, order_id BIGINT NOT NULL UNIQUE, coupon_id BIGINT NOT NULL DEFAULT 0, amount DECIMAL(10,2) NOT NULL, reason VARCHAR(255) NOT NULL DEFAULT '', status TINYINT NOT NULL DEFAULT 0, audit_by BIGINT NOT NULL DEFAULT 0, audit_reason VARCHAR(255) NOT NULL DEFAULT '', channel VARCHAR(32) NOT NULL DEFAULT '', channel_refund_txn VARCHAR(128) NOT NULL DEFAULT '', channel_status VARCHAR(32) NOT NULL DEFAULT '', channel_failure_reason VARCHAR(255) NOT NULL DEFAULT '', audited_at TIMESTAMP NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX IF NOT EXISTS idx_refund_channel_txn ON refund(channel, channel_refund_txn);
CREATE INDEX IF NOT EXISTS idx_refund_processing ON refund(status, updated_at, id);
CREATE TABLE IF NOT EXISTS channel_statement_batch (id BIGINT AUTO_INCREMENT PRIMARY KEY, admin_id BIGINT NOT NULL, region VARCHAR(8) NOT NULL, channel VARCHAR(32) NOT NULL, file_name VARCHAR(255) NOT NULL, file_sha256 CHAR(64) NOT NULL, total_rows INT NOT NULL DEFAULT 0, matched_rows INT NOT NULL DEFAULT 0, discrepancy_rows INT NOT NULL DEFAULT 0, unmatched_rows INT NOT NULL DEFAULT 0, invalid_rows INT NOT NULL DEFAULT 0, ignored_rows INT NOT NULL DEFAULT 0, status TINYINT NOT NULL DEFAULT 0, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT uk_channel_statement_file UNIQUE(region,channel,file_sha256));
CREATE INDEX IF NOT EXISTS idx_channel_statement_batch_region ON channel_statement_batch(region, created_at, id);
CREATE TABLE IF NOT EXISTS channel_statement_item (id BIGINT AUTO_INCREMENT PRIMARY KEY, batch_id BIGINT NOT NULL, line_no INT NOT NULL, transaction_type VARCHAR(32) NOT NULL DEFAULT '', channel_transaction_id VARCHAR(128) NOT NULL DEFAULT '', amount DECIMAL(12,2), currency CHAR(3) NOT NULL DEFAULT '', channel_status VARCHAR(32) NOT NULL DEFAULT '', occurred_at VARCHAR(64) NOT NULL DEFAULT '', order_no VARCHAR(32) NOT NULL DEFAULT '', local_biz_type VARCHAR(16) NOT NULL DEFAULT '', local_biz_id BIGINT NOT NULL DEFAULT 0, local_amount DECIMAL(12,2), local_currency CHAR(3) NOT NULL DEFAULT '', local_status TINYINT, reconcile_status VARCHAR(16) NOT NULL, discrepancy_reason VARCHAR(255) NOT NULL DEFAULT '', raw_data CLOB, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX IF NOT EXISTS idx_channel_statement_item_batch ON channel_statement_item(batch_id, reconcile_status, line_no, id);
