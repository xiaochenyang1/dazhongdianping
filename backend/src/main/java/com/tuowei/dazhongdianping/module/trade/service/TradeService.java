...
@Service public class TradeService {
 ...
 private final TradeMapper mapper;private final UserGrowthService userGrowthService;private final CouponLifecycleService couponLifecycleService;private final NotificationService notificationService;private final String secret;private final PaymentChannelResolver channelResolver;private final com.tuowei.dazhongdianping.module.marketing.service.MarketingCouponService marketingCouponService;public TradeService(TradeMapper mapper,UserGrowthService userGrowthService,CouponLifecycleService couponLifecycleService,NotificationService notificationService,@Value("${app.payment.notify-secret}")String secret,PaymentChannelResolver channelResolver,com.tuowei.dazhongdianping.module.marketing.service.MarketingCouponService marketingCouponService){this.mapper=mapper;this.userGrowthService=userGrowthService;this.couponLifecycleService=couponLifecycleService;this.notificationService=notificationService;this.secret=secret;this.channelResolver=channelResolver;this.marketingCouponService=marketingCouponService;}
 public List<Map<String,Object>> shopDeals(Long shopId){return mapper.selectShopDeals(shopId,region()).stream().map(this::dealSummary).toList();}
 public Map<String,Object> deal(Long id){DealRow d=requireDeal(id);Map<String,Object> m=new LinkedHashMap<>(dealSummary(d));m.put("items",mapper.selectDealItems(id));m.put("rules",d.getRules());m.put("validStart",d.getValidStart());m.put("validEnd",d.getValidEnd());return m;}
 @Transactional public Map<String,Object> createOrder(OrderCreateRequest req){UserSession u=user();DealRow d=requireDeal(req.dealId());if(d.getValidEnd()!=null&&d.getValidEnd().isBefore(LocalDate.now()))throw new IllegalArgumentException("团购已过期");if(mapper.decrementDealStock(d.getId(),req.quantity())==0)throw new IllegalArgumentException("团购库存不足");OrderRow o=new OrderRow();o.setOrderNo("OD"+System.currentTimeMillis()+String.format("%04d",new Random().nextInt(10000)));o.setUserId(u.userId());o.setDealId(d.getId());o.setShopId(d.getShopId());o.setRegion(d.getRegion());o.setQuantity(req.quantity());o.setUnitPrice(d.getPrice());BigDecimal original=d.getPrice().multiply(BigDecimal.valueOf(req.quantity())).setScale(2);com.tuowei.dazhongdianping.module.marketing.service.CouponDiscount discount=marketingCouponService.applyForOrder(u.userId(),req.userCouponId(),d.getShopId(),original);o.setOriginalAmount(original);o.setDiscountAmount(discount.discountAmount());o.setUserCouponId(discount.userCouponId());o.setAmount(original.subtract(discount.discountAmount()).setScale(2));o.setCurrency(d.getCurrency());o.setExpireAt(LocalDateTime.now().plusMinutes(30));mapper.insertOrder(o);if(discount.userCouponId()!=null)marketingCouponService.bindCouponToOrder(discount.userCouponId(),o.getId());return orderMap(requireOrder(o.getId()),false);}
 ...
 /** 下单前预览：给定团购与数量，返回可用营销券列表。 */
 public List<?> usableCoupons(Long dealId,Integer quantity){user();DealRow d=requireDeal(dealId);int q=quantity==null?1:Math.min(20,Math.max(1,quantity));BigDecimal amount=d.getPrice().multiply(BigDecimal.valueOf(q)).setScale(2);return marketingCouponService.usableForOrder(d.getShopId(),amount);}
 ...
 @Transactional public Map<String,Object> cancel(Long id){OrderRow o=requireOrder(id);if(mapper.closeOrder(id,o.getUserId())==0)throw new IllegalArgumentException("订单当前不可取消");mapper.restoreDealStock(o.getDealId(),o.getQuantity());marketingCouponService.releaseForOrder(o.getId());return orderMap(requireOrder(id),false);}
 ...
 private Map<String,Object> orderMap(OrderRow o,boolean includeCoupons){Map<String,Object>m=new LinkedHashMap<>();m.put("id",o.getId());m.put("orderNo",o.getOrderNo());m.put("dealId",o.getDealId());m.put("dealTitle",o.getDealTitle());m.put("shopId",o.getShopId());m.put("shopName",o.getShopName());m.put("coverImage",o.getCoverImage());m.put("quantity",o.getQuantity());m.put("unitPrice",o.getUnitPrice());m.put("originalAmount",o.getOriginalAmount()==null?o.getAmount():o.getOriginalAmount());m.put("discountAmount",o.getDiscountAmount()==null?BigDecimal.ZERO:o.getDiscountAmount());m.put("amount",o.getAmount());m.put("currency",o.getCurrency());m.put("payStatus",o.getPayStatus());m.put("payStatusText",switch(o.getPayStatus()){case 1->"已支付";case 2->"已退款";case 3->"部分退款";default->"待支付";});m.put("status",o.getStatus());RefundRow refund=mapper.selectRefundByOrder(o.getId());if(refund!=null)m.put("refund",refundMap(refund));if(includeCoupons)m.put("coupons",mapper.selectOrderCoupons(o.getId(),o.getUserId()).stream().map(this::couponMap).toList());return m;}
 ...
 private void notifyOrderPaid(OrderRow order){
  if(order==null||order.getUserId()==null)return;
  String dealTitle=order.getDealTitle()==null||order.getDealTitle().isBlank()?"团购订单":order.getDealTitle();
  String amountText=order.getAmount()==null?"":order.getAmount().setScale(2,RoundingMode.HALF_UP).toPlainString();
  String currency=order.getCurrency()==null||order.getCurrency().isBlank()?"":order.getCurrency();
  String content=dealTitle
          +" · 订单 "+order.getOrderNo()
          +(amountText.isBlank()?"":" · "+amountText+(currency.isBlank()?"":" "+currency))
          +" · 券码已发放，可在我的券查看";
  String linkUrl="/user/orders/"+order.getId()+"?paid=1";
  String region=order.getRegion()==null||order.getRegion().isBlank()?RegionContext.getRegion().name():order.getRegion();
  notificationService.create(order.getUserId(),region,ORDER_PAID_TYPE,"支付成功",content,linkUrl);
 }
