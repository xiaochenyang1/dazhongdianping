...
当前 M4 已闭环，M5 商户经营与通知、M6 Flutter 本地业务闭环均已落地。M7 已完成帖子（含转发/取消转发、评论盖楼、帖子正文/评论 `@提醒`）、关注流、APP 私信、区域化官方圈子以及话题广场/7 天热榜：Flutter 可浏览、互动和关注话题，PC Web 只读，管理端可治理、合并和重算热榜，并完成 `topics` 隐私导出/注销治理。真实 FCM/APNs 推送、真实支付 SDK、Google Maps 与目标环境凭证联调仍属于后续阶段。
...
- 当前后端默认运行配置已指向 `MySQL`；可直接导入 MySQL 的脚本已补到 `sql/mysql/`，并带公开点评、点评图片、点赞/评论演示数据、审核演示数据和可直接密码登录的 C 端演示账号。`H2` 仍保留为 `h2` profile 和测试环境使用。
...
3. 继续做 M7 的达人认证和真实推送，并推进真实第三方/目标环境联调；帖子转发、评论盖楼、帖子正文/评论 `@提醒`、关注流、私信、官方圈子和话题广场/热榜已经落地。
