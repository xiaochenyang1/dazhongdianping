...
- `2026-07-28` Flutter 国际化进入逐域收口：根 `MaterialApp` 已注册应用级本地化委托；搜索与发现、城市榜单列表/详情、运营活动列表/详情、消息通知中心、用户中心入口完整链路已支持简体中文、繁体中文和英文；未迁移页面仍保留为完整 i18n 缺口。`flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `408` 条通过。
