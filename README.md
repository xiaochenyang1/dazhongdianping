...
- `2026-07-28` Flutter 国际化进入逐域收口：根 `MaterialApp` 已注册应用级本地化委托；搜索与发现、城市榜单/活动、消息通知、用户中心、社区流、话题、圈子、私信/黑名单、交易订单/券/团购、预订列表/创建/详情完整链路已支持简体中文、繁体中文和英文；未迁移页面仍保留为完整 i18n 缺口。`flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `418` 条通过。
