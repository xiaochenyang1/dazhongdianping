...
- `2026-07-28` Flutter 国际化进入逐域收口：根 `MaterialApp` 已注册应用级本地化委托；搜索与发现、城市榜单列表/详情、运营活动列表/详情、消息通知中心完整链路（标题、空态、错误/重试、刷新失败提示、筛选、全部已读、未读角标、门店/资源计数、榜首文案、外部链接打开失败提示和金额）已支持简体中文、繁体中文和英文；未迁移页面仍保留为完整 i18n 缺口。`flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `407` 条通过。
