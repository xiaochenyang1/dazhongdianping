...
- `2026-07-28` Flutter 国际化进入逐域收口：根 `MaterialApp` 已注册应用级本地化委托，搜索与发现完整链路（标题、输入提示、联想、热词、历史操作、错误/空态、分页和金额）已支持简体中文、繁体中文和英文；未迁移页面仍保留为完整 i18n 缺口。`flutter analyze` 零问题，`flutter test` 全量 `403` 条通过。
