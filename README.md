...
- `admin-web/`: `Vue 3 + TypeScript + Vite` 管理端运营后台，覆盖门店、分类/城市/商圈基础数据、审核、达人认证、榜单、成长、圈子和话题治理，以及审计日志、隐私任务查询。
...
- 管理端数据库 RBAC 基础已完成：管理员、角色、权限点、管理员-角色、角色-权限和管理员区域范围均已落库；`/auth/me` 返回实时身份、权限与 `CN/EU` 范围，菜单、路由和 API 按权限过滤。角色停用后旧 token 仍可访问 `auth/me`，但权限会在下一次请求重新计算并被收回，固定受限 API 返回 `403`，动态审核列表可返回 `200` 空结果；管理员账号停用后旧 token 才会在下一次请求返回 `401`，前端清理 `localStorage` 并回到登录页。`admin-web` 已提供管理员账号、角色权限、审计日志和隐私任务页面。
...
- `2026-07-21` 管理端隐私任务查询本轮已完成全量回归：`backend` 运行 `.\mvnw.cmd -q test`，`286` 条测试通过；`admin-web` 运行 `npm test`，`16` 个测试文件、`39` 条测试通过；`admin-web` 运行 `npm run build` 通过。
