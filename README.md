...
当前 M4 已闭环，M5 商户经营与通知、M6 Flutter 本地业务闭环均已落地。M7 已完成帖子（含转发/取消转发、评论盖楼、帖子正文/评论 `@提醒`）、本地达人认证、认证商户号、关注流、APP 私信、区域化官方圈子以及话题广场/7 天热榜：Flutter 可浏览、互动和关注话题，PC Web 只读，管理端可治理、合并和重算热榜，并完成 `topics` 隐私导出/注销治理。FCM/APNs 服务端适配器已落地，真实凭证、真机推送 smoke、真实支付 SDK、Google Maps 与目标环境凭证联调仍待验收。
...
- 移动推送后端适配器已落地：通知事务提交后异步投递，Android 使用 FCM HTTP v1 服务账号 OAuth2，iOS 使用 APNs token-based ES256；临时 HTTP/网络错误按上限退避重试，确定失效 token 自动停用，站内通知与 WebSocket 继续作为补偿来源。默认 `APP_PUSH_ENABLED=false`，真实凭证和真机 smoke 仍待验收。
