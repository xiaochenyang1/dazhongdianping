...
| Flutter 与真实第三方 | 部分完成 | Flutter 具备区域切换和应用级简体中文/繁体中文/英文委托；首页、搜索与发现、城市榜单列表/详情、运营活动列表/详情、消息通知中心、用户中心入口完整链路已迁移三语言并按当前语言格式化金额，交易/社区/隐私主链路已落地；未配置能力会诚实禁用 | 其余页面完整 i18n、点评翻译、Google Maps、Stripe/PayPal/支付宝/微信、FCM/APNs、邮件短信和内容审核 | 首页/搜索/榜单/活动/通知/用户中心三语言组件测试、Flutter 全量测试以及未配置阻断验证通过；真实 sandbox、凭证和供应商联调仍待验收 |
...
- `2026-07-28` Flutter 用户中心入口三语言迁移完成：标题、退出、资料错误/重试、等级积分成长值文案、账户设置/达人认证/成长流水/私信/黑名单/圈子/收藏集合/足迹/隐私中心入口随 `zh-CN`、`zh-TW`、`en` 切换；字典/用户中心聚焦测试与 `flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `408` 条通过。其余页面硬编码仍属于完整 i18n 待办。
