...
| M7 APP 私信 | 已完成 | `README.md`、`docs/需求文档.md`、`docs/接口设计.md`、`docs/数据库设计.md` | `conversation/message/user_block/message_report` | 1v1 文本、会话列表分页、消息历史向前分页（首次加载失败可重试且不误确认已读），以及 Flutter 分页黑名单管理与解除拉黑；列表保留分页元数据并按 ID 去重，另含举报、WebSocket、`messages` 导出与注销治理；PC Web 无入口 |
