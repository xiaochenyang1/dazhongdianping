...
|---|---|---|---|---|
| M1-M4 用户核心链路 | 已完成（本地口径） | 浏览、搜索、认证、点评、成长、榜单、收藏、交易和预订均有后端、Web 与自动化覆盖；预订到店前 2 小时 / 30 分钟站内提醒已落地（`reservation.reminder` + 时间线 action_type=9）；券码到期前 3 天 / 1 天提醒与过期自动标记已落地（`coupon.reminder` / `coupon.expired` + `remind_status` 去重，PC Web 我的券页支持 status/code 定位）；券码详情 `GET /coupons/:code` 与 PC 二维码详情页已落地；退款审核结果通知 `order.refund.result` 与订单详情取消/退款闭环已落地 | 真实支付归第三方阶段 | 当前自动化、浏览器 E2E 和临时 MySQL 8 smoke 有通过记录；目标环境仍待验收 |
