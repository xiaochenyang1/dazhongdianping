...
- 用户中心已完成我的点评、收藏、订单、券、预订：订单详情支持取消与退款申请并诚实隔离未配置支付；我的券支持按状态筛选，列表读取时会顺带把过期待使用券落成 `status=3`，并从通知 `code` 参数高亮定位；`GET /coupons/:code` 券码详情返回规则、`usable` 与核销二维码，PC Web `/user/coupons/:code` 已接列表/订单入口；券码到期前 3 天 / 1 天会发 `coupon.reminder`，过期后发 `coupon.expired`；预订详情支持取消、查询新时段、改期和时间线。
...
|---|---|---|---|---|
| M1-M4 用户核心链路 | 已完成（本地口径） | 浏览、搜索、认证、点评、成长、榜单、收藏、交易和预订均有后端、Web 与自动化覆盖；预订到店前 2 小时 / 30 分钟站内提醒已落地（`reservation.reminder` + 时间线 action_type=9）；券码到期前 3 天 / 1 天提醒与过期自动标记已落地（`coupon.reminder` / `coupon.expired` + `remind_status` 去重，PC Web 我的券页支持 status/code 定位）；券码详情 `GET /coupons/:code` 与 PC 二维码详情页已落地 | 真实支付归第三方阶段 | 当前自动化、浏览器 E2E 和临时 MySQL 8 smoke 有通过记录；目标环境仍待验收 |
