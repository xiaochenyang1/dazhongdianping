...
| Flutter 与真实第三方 | 部分完成 | Flutter 具备区域切换和应用级简体中文/繁体中文/英文委托；首页搜索、榜单活动、通知、用户中心、社区话题圈子、私信黑名单、交易预订点评认证、隐私账户达人成长、公开主页/收藏/足迹/门店详情点评/帖子详情编辑完整链路已迁移三语言，英文动态计数已覆盖单复数；未配置能力会诚实禁用 | 点评翻译、Google Maps、Stripe/PayPal/支付宝/微信、FCM/APNs、邮件短信和内容审核 | 多域三语言组件测试、Flutter 全量测试以及未配置阻断验证通过；真实 sandbox、凭证和供应商联调仍待验收 |
