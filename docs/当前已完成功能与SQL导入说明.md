...
| Flutter 与真实第三方 | 部分完成 | Flutter 具备区域切换和应用级简体中文/繁体中文/英文委托；首页、搜索、榜单/活动、通知、用户中心、社区、话题、圈子、私信/黑名单、交易、预订、点评详情/编辑完整链路已迁移三语言；未配置能力会诚实禁用 | 其余页面完整 i18n、点评翻译、Google Maps、Stripe/PayPal/支付宝/微信、FCM/APNs、邮件短信和内容审核 | 多域三语言组件测试、Flutter 全量测试以及未配置阻断验证通过；真实 sandbox、凭证和供应商联调仍待验收 |
...
- `2026-07-28` Flutter 点评链路三语言迁移完成：写点评/编辑、详情点赞评论举报删除、审核备注/商家回复、评论区与图片上传提示随 `zh-CN`、`zh-TW`、`en` 切换；字典/点评聚焦测试与 `flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `418` 条通过。其余页面硬编码仍属于完整 i18n 待办。
