...
| Flutter 与真实第三方 | 部分完成 | Flutter 具备区域切换和应用级简体中文/繁体中文/英文委托；首页、搜索与发现、城市榜单列表/详情、运营活动列表/详情完整链路已迁移三语言并按当前语言格式化金额，交易/社区/隐私主链路已落地；未配置能力会诚实禁用 | 其余页面完整 i18n、点评翻译、Google Maps、Stripe/PayPal/支付宝/微信、FCM/APNs、邮件短信和内容审核 | 首页/搜索/榜单/活动三语言组件测试、Flutter 全量测试以及未配置阻断验证通过；真实 sandbox、凭证和供应商联调仍待验收 |
...
- `2026-07-28` Flutter 城市榜单与运营活动三语言迁移完成：列表/详情标题、空态、错误/重试、刷新失败提示、门店/资源计数、榜首文案、外部链接打开失败提示和榜单金额随 `zh-CN`、`zh-TW`、`en` 切换；字典/榜单活动聚焦测试与 `flutter analyze` 零问题，`flutter test --concurrency=1` 全量 `406` 条通过。其余页面硬编码仍属于完整 i18n 待办。
