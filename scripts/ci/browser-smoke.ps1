...
$runDir = Join-Path ([System.IO.Path]::GetTempPath()) ("dazhongdianping-browser-smoke-" + [System.Guid]::NewGuid().ToString("N"))
$succeeded = $false

function Write-Step {
    param([string]$Message)
    Write-Output "[browser-smoke] $Message"
}
...
if ($DryRun) {
    Write-Output "Plan:"
    Write-Output "1. Start web Vite directly with Node on http://127.0.0.1:$WebPort."
    Write-Output "2. Start admin-web Vite directly with Node on http://127.0.0.1:$AdminPort."
    Write-Output "3. Run Chromium against web/e2e/browser-smoke.spec.ts with externally managed dev servers."
    Write-Output "4. Smoke web /, /shops, /shops/1/reviews/new, and admin-web /login."
    Write-Output "5. Stop every managed process and clean temporary logs on success."
    exit 0
}
...
$previousExecutablePath = $env:PLAYWRIGHT_EXECUTABLE_PATH
...
try {
    ...
    if ($ChromePath) {
        ...
        $env:PLAYWRIGHT_EXECUTABLE_PATH = (Resolve-Path -LiteralPath $ChromePath).Path
    }
    ...
    Write-Step "running Playwright browser smoke"
    ...
    $succeeded = $true
    Write-Step "browser smoke passed"
}
finally {
    ...
    Restore-EnvironmentVariable -Name "PLAYWRIGHT_EXECUTABLE_PATH" -Value $previousExecutablePath
    ...
    elseif (Test-Path -LiteralPath $runDir) {
        Write-Warning "Browser smoke logs were preserved at $runDir"
    }
}
