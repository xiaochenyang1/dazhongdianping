...
Assert-True ($dryRunText -match "SHA-256") "dry-run output must mention downloaded content verification"
...
Assert-True ($scriptContent -match 'GetAsync\(\$Url\)') "storage smoke must download the returned public URL"
Assert-True ($scriptContent -match "Get-ByteArraySha256") "storage smoke must compare downloaded content hashes"
Assert-True ($scriptContent -match '/api/c/v1/files/\$encodedFileName') "storage smoke must verify non-HTTP objects through the backend S3 reader"
