...
const zhCnStrings = {
  ...
  stats: (level: number, points: number, growth: number) => `Lv.${level} · ${points} 积分 · ${growth} 成长值`, saving: '保存中...', saveProfile: '保存资料', dailyCheckIn: '每日签到', openCheckIn: '去签到', pointsMall: '积分商城', openPointsMall: '去兑换', growthHistory: '查看成长值流水', privacyCenter: '进入隐私中心',
...
const enStrings = {
  ...
  stats: (level: number, points: number, growth: number) => `Lv.${level} · ${formatEnglishCount(points, 'point')} · ${growth} growth`, saving: 'Saving...', saveProfile: 'Save profile', dailyCheckIn: 'Daily check-in', openCheckIn: 'Open check-in', pointsMall: 'Points mall', openPointsMall: 'Open mall', growthHistory: 'View growth history', privacyCenter: 'Open privacy centre',
