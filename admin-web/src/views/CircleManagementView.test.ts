...
describe('CircleManagementView', () => {
  ...
  it('loads current-region circles and disables one', async () => {
    ...
    expect(host.textContent).toContain('Official circles')
    const button=[...host.querySelectorAll('button')].find((node)=>node.textContent?.includes('Disable'))
    if(!button) throw new Error('missing disable button'); button.click(); await flush()
  ...
  it('updates an existing circle when write permission is available', async () => {
    ...
    const {host,app}=mount(); await flush()
    const editButton=[...host.querySelectorAll('button')].find((node)=>node.textContent?.trim()==='Edit')
    if(!editButton) throw new Error('missing edit button'); editButton.click(); await nextTick()
    const name=host.querySelector<HTMLInputElement>('input[name="circle-name"]'); if(!name) throw new Error('缺少名称输入框')
  ...
  it('keeps circle browsing available while hiding every write control from read-only users', async () => {
    ...
    expect(host.textContent).toContain('伦敦生活圈')
    expect(host.textContent).toContain('Members 12 · Posts 8 · Sort 20')
    expect(host.querySelector('[data-testid="circle-editor"]')).toBeNull()
    expect([...host.querySelectorAll('button')].map((button)=>button.textContent?.trim())).toEqual(['Search'])
  ...
  it('blocks stale mutation handlers after write permission is revoked', async () => {
    const {host,app}=mount(); await flush()
    const editButton=[...host.querySelectorAll<HTMLButtonElement>('button')].find((node)=>node.textContent?.trim()==='Edit')
    const toggleButton=[...host.querySelectorAll<HTMLButtonElement>('button')].find((node)=>node.textContent?.trim()==='Disable')
    if(!editButton||!toggleButton) throw new Error('missing circle write controls')
    editButton.click(); await nextTick()
    const form=host.querySelector<HTMLFormElement>('[data-testid="circle-editor"]')
    if(!form) throw new Error('missing circle editor form')
