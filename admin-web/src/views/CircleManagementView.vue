...
function edit(row:AdminCircle){if(!canWrite.value)return;editingId.value=row.id;Object.assign(form,{name:row.name,description:row.description,coverUrl:row.coverUrl,sort:row.sort})}
function reset(){editingId.value=null;Object.assign(form,{name:'',description:'',coverUrl:'',sort:0})}
async function save(){if(!canWrite.value)return;error.value='';try{const payload={...form};if(editingId.value)await updateCircle(editingId.value,payload);else await createCircle(payload);reset();await load()}catch(e){error.value=e instanceof Error?e.message:'保存失败'}}
async function toggle(row:AdminCircle){if(!canWrite.value)return;error.value='';try{await updateCircleStatus(row.id,row.status===1?2:1);await load()}catch(e){error.value=e instanceof Error?e.message:'状态更新失败'}}
onMounted(load)
</script>
